...
public:
    ...
    void show_video_frame(video_trimmer::graphics::Graphics* graphics, std::unique_ptr<video_content_provider::frame::Frame> video_frame);

    [[nodiscard]] ImageSize size() const { return view_size_; }

private:
    std::unique_ptr<video_content_provider::frame::Frame> current_video_frame_;
